 ...
 *
 * Disassembly of dsdt.dat, Sat Apr  3 03:42:59 2021
 *
 * Original Table Header:
 *     Signature        "DSDT"
 *     Length           0x0001CFC8 (118728)
 *     Revision         0x02
 *     Checksum         0xD8
 *     OEM ID           "FUJ   "
 *     OEM Table ID     "D3410-B1"
 *     OEM Revision     0x11400000 (289406976)
 *     Compiler ID      "INTL"
 *     Compiler Version 0x20120913 (538052883)
 */
DefinitionBlock ("", "DSDT", 2, "FUJ   ", "D3410-B1", 0x11400000)
{
    /*
     * iASL Warning: There were 16 external control methods found during
     * disassembly, but only 6 were resolved (10 unresolved). Additional
    ...
    External (_SB_.PCI0.LPCB.FJEN, IntObj)
    ...
    Name (TPMF, One)
    Name (FJSN, 0xE4)
    Name (PSUS, 0xFDAE0760)
    Name (LLED, 0xFDAE07E0)
    Name (SLED, 0xFDAF04B0)
    Name (PEER, Zero)
    Name (LPAM, Zero)
    Name (POWR, Zero)
    Name (SS1, One)
    ...
    Name (VGAF, One)
    OperationRegion (GNVS, SystemMemory, 0xCBB7B000, 0x0601)
    ...
    Scope (_SB)
    {
        Device (PCI0)
        {
            ...
            Device (LPCB)
            {
                ...
                OperationRegion (CPSB, SystemMemory, 0xC7C3DF18, 0x10)
                ...
                Device (SIO1)
                {
                    ...
                    Method (DCNT, 2, NotSerialized)
                    {
                        ...
                        If ((Arg1 == Zero))
                        {
                            ...
                            {
                                OperationRegion (MCR1, SystemIO, 0x03FC, One)
                                Field (MCR1, ByteAcc, NoLock, Preserve)
                                {
                                    DTR1,   1, 
                                    RTS1,   1
                                }

                                OperationRegion (MCR2, SystemIO, 0x02FC, One)
                                Field (MCR2, ByteAcc, NoLock, Preserve)
                                {
                                    DTR2,   1, 
                                    RTS2,   1
                                }

                                OperationRegion (MCR3, SystemIO, 0x03EC, One)
                                Field (MCR3, ByteAcc, NoLock, Preserve)
                                {
                                    DTR3,   1, 
                                    RTS3,   1
                                }

                                OperationRegion (MCR4, SystemIO, 0x02EC, One)
                                Field (MCR4, ByteAcc, NoLock, Preserve)
                                {
                                    DTR4,   1, 
                                    RTS4,   1
                                }
                    ...
                    Method (SIOS, 1, NotSerialized)
                    {
                        ...
                        If ((0x05 != Arg0))
                        {
                            ...
                            If (KBFG)
                            {
                                If ((Arg0 < 0x04))
                                {
                                    Local0 = RGE0 /* \_SB_.PCI0.LPCB.SIO1.RGE0 */
                                    RGE0 = (0x41 | Local0)
                                }
                                Else
                                {
    ...
    Scope (_SB.PCI0)
    {
        Device (XHC)
        {
            ...
            Method (_PRW, 0, NotSerialized)  // _PRW: Power Resources for Wake
            {
                If (CondRefOf (\_SB.PCI0.LPCB.FJEN))
                {
                    If (^^LPCB.FJEN)
                    {
    ...
    Scope (_SB.PCI0)
    {
        ...
        Device (SAT0)
        {
            ...
            {
                Return (SATS) /* \_SB_.PCI0.SAT0.SATS */
            }
    ...
    Scope (_SB.PCI0)
    {
        ...
        Device (GPI0)
        {
            ...
            Method (_CRS, 0, NotSerialized)  // _CRS: Current Resource Settings
            {
                ...
                CreateDWordField (RBUF, \_SB.PCI0.GPI0._CRS._Y31._INT, IRQN)  // _INT: Interrupts
                COM0 = (SBRG + 0x00AF0000)
                COM1 = (SBRG + 0x00AE0000)
                CreateDWordField (RBUF, \_SB.PCI0.GPI0._CRS._Y32._BAS, COM3)  // _BAS: Base Address
    ...
    Scope (_SB.PCI0.LPCB)
    {
        ...
        Device (HPET)
        {
            ...
            Method (_CRS, 0, Serialized)  // _CRS: Current Resource Settings
            {
                If (HPTE)
                {
                    CreateDWordField (BUF0, \_SB.PCI0.LPCB.HPET._Y3E._BAS, HPT0)  // _BAS: Base Address
                    HPT0 = HPTB /* \HPTB */
                }
    ...
    Device (_SB.TPM)
    {
        ...
        Method (_CRS, 0, Serialized)  // _CRS: Current Resource Settings
        {
            If ((AMDT == One))
            {
                CreateDWordField (CRST, \_SB.TPM._Y3F._BAS, MTFB)  // _BAS: Base Address
                CreateDWordField (CRST, \_SB.TPM._Y3F._LEN, LTFB)  // _LEN: Length
                MTFB = TPMB /* \TPMB */
                LTFB = 0x1000
                CreateDWordField (CRST, \_SB.TPM._Y40._BAS, MTFC)  // _BAS: Base Address
                CreateDWordField (CRST, \_SB.TPM._Y40._LEN, LTFC)  // _LEN: Length
                MTFC = TPMC /* \TPMC */
                LTFC = 0x1000
            ...
            Else
            {
                If ((DTPT == One))
                {
                    CreateDWordField (CRSD, \_SB.TPM._Y41._BAS, MTFE)  // _BAS: Base Address
                    CreateDWordField (CRSD, \_SB.TPM._Y41._LEN, LTFE)  // _LEN: Length
                    MTFE = 0xFED40000
                    LTFE = 0x0880
                ...
                ElseIf ((TTPF == One))
                {
                    CreateDWordField (CRSI, \_SB.TPM._Y42._BAS, MTFD)  // _BAS: Base Address
                    CreateDWordField (CRSI, \_SB.TPM._Y42._LEN, LTFD)  // _LEN: Length
                    MTFD = 0xFED40000
                    LTFD = 0x5000
                ...
                ElseIf ((TTPF == Zero))
                {
                    CreateDWordField (CRSI, \_SB.TPM._Y42._BAS, MTFF)  // _BAS: Base Address
                    MTFF = TPMM /* \TPMM */
                    Return (CRSI) /* \_SB_.TPM_.CRSI */
                }
    ...
    Scope (_SB.PCI0.LPCB)
    {
        Device (FEXT)
        {
            Name (_HID, EisaId ("FUJ02E3"))  // _HID: Hardware ID
            Name (\FJPR, 0xCBB66F18)
            OperationRegion (FCOR, SystemMemory, FJPR, One)
            ...
            Name (\FJCB, 0xCBB66F98)
            OperationRegion (FCOP, SystemMemory, FJCB, 0x14)
            Field (FCOP, AnyAcc, NoLock, Preserve)
            {
                FAR0,   32, 
                FAR1,   32, 
                FAR2,   32, 
            ...
            OperationRegion (SRB0, SystemIO, 0xB2, One)
            ...
            Method (FUNC, 4, Serialized)
            {
                ...
                FAR3 = Arg3
                SRSM = FJSN /* \FJSN */
                Return (FRTV) /* \_SB_.PCI0.LPCB.FEXT.FRTV */
            }
        }
    }
    ...
    Scope (_SB.PCI0)
    {
        OperationRegion (LPM2, SystemMemory, LLED, 0x04)
        Field (LPM2, AnyAcc, NoLock, Preserve)
        {
            LLST,   1
        }

        OperationRegion (LPM3, SystemMemory, SLED, 0x04)
        Field (LPM3, AnyAcc, NoLock, Preserve)
        {
            SLST,   1
        }

        OperationRegion (LPM4, SystemMemory, PSUS, 0x04)
        Field (LPM4, AnyAcc, NoLock, Preserve)
        {
            PSUT,   1, 
            PSUR,   1, 
            Offset (0x01), 
    ...
    Scope (_SB)
    {
        Device (AMW1)
        {
            ...
            {
                /* 0000 */  0xB8, 0xED, 0x69, 0x84, 0x92, 0xD3, 0xB4, 0x4F,  // ..i....O
                /* 0008 */  0xA8, 0xB7, 0xB0, 0xDC, 0x4F, 0x21, 0x81, 0xA5,  // ....O!..
                /* 0010 */  0x41, 0x41, 0x01, 0x01, 0xA2, 0xA1, 0xDF, 0x0A,  // AA......
                /* 0018 */  0x0F, 0xA0, 0x80, 0x41, 0xB0, 0x3E, 0x1D, 0x95,  // ...A.>..
                /* 0020 */  0x23, 0xEB, 0x9E, 0x73, 0x41, 0x42, 0x01, 0x01,  // #..sAB..
                /* 0028 */  0xF3, 0x9E, 0xFF, 0x93, 0xBD, 0xE6, 0x04, 0x41,  // .......A
                /* 0030 */  0x97, 0xAA, 0xCA, 0x30, 0x0F, 0x93, 0xCE, 0x43,  // ...0...C
                /* 0038 */  0x41, 0x43, 0x01, 0x01, 0x48, 0xE7, 0x93, 0x26,  // AC..H..&
                /* 0040 */  0x93, 0xC9, 0xB2, 0x44, 0xB6, 0x38, 0xA7, 0x33,  // ...D.8.3
                /* 0048 */  0x91, 0x04, 0xDA, 0xD5, 0x41, 0x44, 0x01, 0x01   // ....AD..
            })
            ...
            {
                CreateByteField (Arg1, Zero, PSUV)
                If ((LPSU == One))
                {
                    ^^PCI0.PSUT = ~PSUV
                }
